...
- 작업 카드의 `owned_files`에 포함된 경로만 직접 수정한다.
- `shared_files`는 카드가 지정한 통합 작업자만 수정한다.
- 병렬 그룹에 속한 카드끼리 `owned_files`가 겹치면 병렬 실행하지 않는다.
- 범위에 없는 공통 파일 변경이 필요하면 먼저 별도 통합 카드를 만들거나 기존 카드의 소유권을 갱신한다.
