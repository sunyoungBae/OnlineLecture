...
`상태`는 카드 생성 시 초기값이다. P00 작업은 하네스 구축 후 `done`, P01-T01은 `ready`, 나머지는 선행 작업 완료 전 `blocked`다.
...
| P06-T02 | 게시글 첨부 업로드·서명 다운로드·정리 | blocked | P05-T03,P06-T01 | U-B | 게시글 첨부 서버 로직 | 게시글 편집·본문 통합 |
| P06-T03 | 회차 자료 업로드·다운로드·삭제 | blocked | P04-T03,P06-T01 | U-B | 회차 첨부 서버 로직 | 회차 관리자·플레이어 통합 |
