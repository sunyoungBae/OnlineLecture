...
| P04-T03 | 운영자 회차 CRUD·위아래 순서 이동 | blocked | P04-T02 | - | 회차 관리·액션 및 서버 페이지 테스트·순서 로직·관리자 전용 원자 이동 RPC migration·pgTAP·DB 타입 | 강의 관리자 화면과 생성 DB 타입 통합 소유 |
...
| P05-T02 | 공개 목록·escaped ILIKE 검색·필터·페이지네이션 | blocked | P05-T01 | - | 게시판 조회 라우트·서버 페이지 테스트·검색 파서 | 게시판 내비게이션 |
| P05-T03 | 댓글 CRUD·공지·운영자 삭제 | blocked | P05-T02 | - | 댓글·관리 액션 테스트와 게시글 상세 서버 페이지 테스트 | 게시글 본문 화면 |
...
| P06-T04 | 80% Resend 1회·재무장·95% 차단/복구 | blocked | P06-T02,P06-T03 | - | 관리자 전용 원자 usage/claim RPC migration·pgTAP, 경고 발송, 게시글·회차 업로드/삭제 통합과 관리자 저장량 페이지 테스트 | storage_settings·첨부 서버 경로·생성 DB 타입 통합 소유 |
