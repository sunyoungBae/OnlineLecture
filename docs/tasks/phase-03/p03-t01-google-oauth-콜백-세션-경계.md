...
Google OAuth·콜백·세션 경계을 완료해 다음 의존 작업이 사용할 검증된 산출물을 제공한다.

# 범위

- 생성 또는 수정: src/app/auth/callback/route.ts
...
3. 다음 작업 전 검사를 실행한다: npm run test:e2e -- tests/e2e/auth.spec.ts
4. 기대 결과를 확인한다: OAuth 콜백과 리디렉션이 없어 실패.
5. 범위 안의 최소 변경만 구현한다.
6. 다음 통과 검사를 실행한다: npm run test:e2e -- tests/e2e/auth.spec.ts && npm run typecheck
...
- 작업 전 명령: npm run test:e2e -- tests/e2e/auth.spec.ts
- 예상 작업 전 결과: OAuth 콜백과 리디렉션이 없어 실패
- 완료 명령: npm run test:e2e -- tests/e2e/auth.spec.ts && npm run typecheck
- 기대 완료 결과: 기본 신원 범위 로그인과 세션 콜백이 동작하고 서버 키가 노출되지 않는다.

# 인수 조건

- 기본 신원 범위 로그인과 세션 콜백이 동작하고 서버 키가 노출되지 않는다.
- 정상 경로와 거부 경계가 검증된다.
...
권장 메시지: 기능: Google OAuth·콜백·세션 경계
