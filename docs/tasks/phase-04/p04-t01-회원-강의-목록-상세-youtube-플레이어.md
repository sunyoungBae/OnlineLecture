...
2. 다음 작업 전 검사를 실행한다: npm run test -- src/features/courses/youtube.test.ts
3. 기대 결과를 확인한다: YouTube URL 파서가 없어 실패.
4. 범위 안의 최소 변경만 구현한다.
5. 다음 통과 검사를 실행한다: npm run test -- src/features/courses/youtube.test.ts && npm run typecheck
