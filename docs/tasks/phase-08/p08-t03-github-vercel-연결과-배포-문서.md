...
GitHub·Vercel 연결과 배포 문서을 완료해 다음 의존 작업이 사용할 검증된 산출물을 제공한다.
...
2. 다음 작업 전 검사를 실행한다: test -f docs/operations/deployment.md
3. 기대 결과를 확인한다: 배포 절차가 없어 실패.
4. 범위 안의 최소 변경만 구현한다.
5. 다음 통과 검사를 실행한다: npm run build && test -s docs/operations/deployment.md
...
- 증거 유형: 구조/명령
- 작업 전 명령: test -f docs/operations/deployment.md
- 예상 작업 전 결과: 배포 절차가 없어 실패
- 완료 명령: npm run build && test -s docs/operations/deployment.md
- 기대 완료 결과: GitHub·Vercel 연결, 리디렉션, 환경 변수와 무료 비상업 조건이 기록된다.

# 인수 조건

- GitHub·Vercel 연결, 리디렉션, 환경 변수와 무료 비상업 조건이 기록된다.
- 정상 경로와 거부 경계가 검증된다.
