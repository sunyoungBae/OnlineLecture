...
| P00-T04 | 31개 작업 카드와 대시보드 | review | - |
| P00-T05 | 하네스 자동 검사 | blocked | - |
| P01-T01 | Next.js·TypeScript·Tailwind·Vitest·Playwright 기반 | blocked | - |
...
- 진행 중: 없음
- 리뷰 대기: 31개 작업 카드와 대시보드
- 다음 실행 가능: 작업 카드 리뷰 완료 후 하네스 검사기
- 외부 차단: 배포 인수 2건은 배포와 외부 계정 준비 후 해제

## 상태 변경 체크

1. 카드의 의존성과 파일 소유권을 확인한다.
2. 카드 front matter와 이 표를 같은 커밋에서 갱신한다.
3. 하네스 검사 명령을 실행한다. 검사기가 아직 없는 카드 작성 단계에서는 카드 수와 필수 키를 수동 검사한다.
