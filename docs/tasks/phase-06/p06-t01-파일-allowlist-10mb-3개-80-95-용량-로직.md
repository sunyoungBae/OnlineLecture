...
2. 다음 작업 전 검사를 실행한다: npm run test -- src/features/attachments/validation.test.ts src/features/attachments/quota.test.ts
3. 기대 결과를 확인한다: 파일·용량 검증 모듈이 없어 실패.
4. 범위 안의 최소 변경만 구현한다.
5. 다음 통과 검사를 실행한다: npm run test -- src/features/attachments/validation.test.ts src/features/attachments/quota.test.ts
