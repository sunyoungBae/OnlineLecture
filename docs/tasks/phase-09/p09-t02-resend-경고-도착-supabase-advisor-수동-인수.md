---
id: P09-T02
title: Resend 경고 도착·Supabase Advisor 수동 인수
status: blocked
...
Resend 경고 도착·Supabase Advisor 수동 인수을 완료해 다음 의존 작업이 사용할 검증된 산출물을 제공한다.
...
2. 다음 작업 전 검사를 실행한다: test -s docs/acceptance/email-advisor.md
3. 기대 결과를 확인한다: 이메일·Advisor 수동 증거가 없어 실패.
4. 범위 안의 최소 변경만 구현한다.
5. 다음 통과 검사를 실행한다: test -s docs/acceptance/email-advisor.md
...
- 80% 이메일 수신, 재무장과 Advisor 조치·승인자가 기록된다.
- 정상 경로와 거부 경계가 검증된다.
