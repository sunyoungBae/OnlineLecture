...
shared_files: ["src/components", "src/app"]
reviewer: ""
review_commit: ""
---
...
- 통합 소유 시에만 수정: src/components
- 통합 소유 시에만 수정: src/app
...
2. 다음 작업 전 검사를 실행한다: npm run test:e2e -- tests/e2e/accessibility.spec.ts tests/e2e/responsive.spec.ts
3. 기대 결과를 확인한다: 접근성·반응형 위반으로 실패.
4. 범위 안의 최소 변경만 구현한다.
5. 다음 통과 검사를 실행한다: npm run test:e2e -- tests/e2e/accessibility.spec.ts tests/e2e/responsive.spec.ts
