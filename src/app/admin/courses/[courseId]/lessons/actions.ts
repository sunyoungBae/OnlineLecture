...
export type LessonMutationClient = {
  ...
  from: (table: "lessons") => {
    delete: () => { eq: (column: "id", value: string) => PromiseLike<MutationResult> };
    ...
    update: (values: { description: string; title: string; youtube_video_id: string }) => { eq: (column: "id", value: string) => PromiseLike<MutationResult> };
  };
};
...
export async function updateLesson(formData: FormDataLike, clientFactory: ClientFactory = defaultClientFactory) {
  ...
  try { const { error } = await (await clientFactory()).from("lessons").update(lesson).eq("id", lessonId); return finish(formData, error ? "error=save" : "notice=lesson-updated"); } catch { return finish(formData, "error=save"); }
}
...
export async function deleteLesson(formData: FormDataLike, clientFactory: ClientFactory = defaultClientFactory) {
  ...
  try { const { error } = await (await clientFactory()).from("lessons").delete().eq("id", lessonId); return finish(formData, error ? "error=save" : "notice=lesson-deleted"); } catch { return finish(formData, "error=save"); }
}
