...
export function lessonFeedback({ error, notice }: { error?: string; notice?: string }) {
  ...
  if (notice === "attachment-uploaded") return { role: "status" as const, text: "자료를 업로드했습니다." };
  if (notice === "attachment-deleted") return { role: "status" as const, text: "자료를 삭제했습니다." };
  if (error === "attachment-invalid") return { role: "alert" as const, text: "자료의 형식, 크기 또는 개수를 확인해 주세요." };
  if (error === "attachment-save") return { role: "alert" as const, text: "자료를 저장하지 못했습니다. 잠시 후 다시 시도해 주세요." };
  if (error === "attachment-delete") return { role: "alert" as const, text: "자료를 삭제하지 못했습니다. 잠시 후 다시 시도해 주세요." };
  return null;
}
...
export default async function LessonAdminPage({ params, searchParams }: { params: Promise<{ courseId: string }>; searchParams: Promise<{ error?: string; notice?: string }> }) {
  ...
  if (!course) notFound();
  const { data: lessons, error } = await client.from("lessons").select("id, title, description, position, youtube_video_id, attachments (id, original_filename, size_bytes)").eq("course_id", course.id).order("position", { ascending: true });
  if (error) return <main className="py-10"><p role="alert">회차를 불러오지 못했습니다.</p></main>;
  const direction = "";
  return <main className="py-10"><Link className="inline-flex min-h-11 items-center focus-visible:outline-2 focus-visible:outline-offset-2" href="/admin/courses">강의 관리로</Link><h2 className="mt-4 text-3xl font-semibold">{course.title} 회차 관리</h2>{feedback ? <p role={feedback.role}>{feedback.text}</p> : null}<form action={createLesson} className="mt-8 space-y-3"><input name="course_id" type="hidden" value={course.id}/><input name="position" type="hidden" value={(lessons?.length ?? 0) + 1}/><label>제목<input name="title" required /></label><label>YouTube URL<input name="youtube_url" required type="url" /></label><label>설명<textarea name="description" /></label><button type="submit">회차 추가</button></form><ol className="mt-10 space-y-8">{(lessons ?? []).map((lesson, index) => <li key={lesson.id} className="border-t pt-4"><form action={updateLesson} className="space-y-3"><input name="course_id" type="hidden" value={course.id}/><input name="lesson_id" type="hidden" value={lesson.id}/><label>제목<input defaultValue={lesson.title} name="title" required /></label><label>YouTube URL<input defaultValue={`https://youtu.be/${lesson.youtube_video_id}`} name="youtube_url" required type="url" /></label><label>설명<textarea defaultValue={lesson.description} name="description" /></label><button type="submit">수정</button></form><section aria-labelledby={`lesson-files-${lesson.id}`} className="mt-4"><h3 id={`lesson-files-${lesson.id}`}>자료</h3><form action={uploadLessonAttachmentAction} className="mt-2 flex flex-wrap items-center gap-2" encType="multipart/form-data"><input name="course_id" type="hidden" value={course.id}/><input name="lesson_id" type="hidden" value={lesson.id}/><label>자료 파일<input accept=".jpg,.jpeg,.png,.webp,.gif,.pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.zip" multiple name="files" required type="file" /></label><button type="submit">자료 업로드</button></form>{lesson.attachments?.length ? <ul className="mt-2">{lesson.attachments.map((attachment) => <li key={attachment.id} className="flex items-center gap-2"><span>{attachment.original_filename} ({attachment.size_bytes} bytes)</span><form action={downloadLessonAttachmentAction}><input name="attachment_id" type="hidden" value={attachment.id}/><button type="submit">다운로드</button></form><form action={deleteLessonAttachmentAction}><input name="course_id" type="hidden" value={course.id}/><input name="attachment_id" type="hidden" value={attachment.id}/><button type="submit">자료 삭제</button></form></li>)}</ul> : <p className="mt-2 text-sm text-muted-foreground">등록된 자료가 없습니다.</p>}</section><div className="mt-3 flex gap-2"><MoveForm courseId={course.id} disabled={index === 0} direction="up" lessonId={lesson.id}/><MoveForm courseId={course.id} disabled={index === (lessons?.length ?? 0) - 1} direction="down" lessonId={lesson.id}/><form action={deleteLesson}><input name="course_id" type="hidden" value={course.id}/><input name="lesson_id" type="hidden" value={lesson.id}/><input name="direction" type="hidden" value={direction}/><button type="submit">삭제</button></form></div></li>)}</ol></main>;
}
async function uploadLessonAttachmentAction(formData: Parameters<typeof uploadLessonAttachments>[0]) { "use server"; return uploadLessonAttachments(formData); }
async function downloadLessonAttachmentAction(formData: Parameters<typeof downloadLessonAttachment>[0]) { "use server"; return downloadLessonAttachment(formData); }
async function deleteLessonAttachmentAction(formData: Parameters<typeof deleteLessonAttachment>[0]) { "use server"; return deleteLessonAttachment(formData); }
