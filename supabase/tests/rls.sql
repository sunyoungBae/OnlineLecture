...
select results_eq(
  $$ select nickname from public.profiles where id = '10000000-0000-0000-0000-000000000001' $$,
  $$ values ('작성자'::text) $$,
  '비회원은 공개 별명을 읽을 수 있다'
);
select results_eq(
...
select lives_ok(
  $$ insert into public.comments (post_id, author_id, body) values ('40000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000004', '내 댓글') $$,
...
select lives_ok(
  $$ insert into public.courses (title, slug) values ('운영자 강의', 'admin-course') $$,
  '운영자는 강의를 만들 수 있다'
);
select lives_ok(
  $$ insert into public.lessons (course_id, position, title, youtube_video_id) values ('20000000-0000-0000-0000-000000000002', 2, '운영자 회차', 'admin-video') $$,
  '운영자는 회차를 만들 수 있다'
);
select lives_ok(
